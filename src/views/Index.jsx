...
useEffect(() => {
  ...
  if(mode === 'daily'){
    const fetchWebcamFeed = async () => {
      try{
          const response = await fetch('http://localhost:3000/dailyloc') // GRNCEK ENDPOINT
          if (!response.ok) {
            throw new Error('Failed to fetch temperature data');
          }
        const data = await response.json();
...
// },[]);
  ...
  return (
    ...
    <div className='w-full h-full min-h-screen bg-slate-100 flex flex-col justify-start items-center font-Reddit-mono'>
      ...
      <Game webcamFeed={webcamFeed} sentAnswer={answerData} username={username} modeData={handleMode}/>
    </div>
    </>
  )
}
